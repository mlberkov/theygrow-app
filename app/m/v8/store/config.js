...
export const STORE_CONFIG = Object.freeze({
    ...
    //
    // changed_in: FIU-DL-001 — VALUE UNCHANGED, and the knob finally means what
    // it says. From L1-P2 to L2 nothing ever set `clean_shutdown` back to 1, so
    // "only when the previous run did not close cleanly" evaluated true at every
    // launch and this knob bought nothing at all — the very cost the line above
    // rejects, paid anyway. The missing half was the call to closeStore(), which
    // store/boot.js parkNativeStore now makes when the page goes hidden.
    ...
    // re-indexing on the same path would be paid at every start that owes one.
    //
    // changed_in: FIU-DL-001 — VALUE UNCHANGED, and one clause of the argument
    // above corrected because this packet falsified it. It used to read "and,
    // because closeStore() is never called, that already happens on effectively
    // every launch". closeStore() IS called now (store/boot.js
    // parkNativeStore), so the integrity_check is back to what LSC-DL-002
    // declared it: owed only after a run that did not close cleanly. The
    // rejection stands on the ground that survives — the open path is the wrong
    // home for index maintenance because a start that DOES owe an integrity
    // scan is exactly the start that should not also owe a full re-tokenise —
    // and the trigger designed in (e) is unchanged.
    //
