// App entry (A1-P4) — the module the shell loads.
//
// Everything that is not core/ still lives here: the readiness/graph renderers,
// the table, the ZPD filter, the three modals, the mobile accordion and init().
// Per-surface feature modules are A1-P5; this packet's job was the core/
// boundary and the move to module scope.
//
...
// the foot of this file is therefore still registered in time. The import graph
// is deliberately flat (this file -> core/*, no barrel) so the five core files
// fetch in parallel rather than in a chain.
...
import {
    ...
    readAccordionStatesRaw,
    writeAccordionStatesJson,
    removeOrphanedAgeFilter,
} from './core/storage.js';
import { setRowZpdReadiness, categoryKey, getSkillRowsForCategory } from './core/dom-utils.js';
import {
    calculateAge,
    calculateCurrentLifeMonth,
    formatAge,
    formatParentActivities,
} from './core/format.js';
...
function openSkillModal(skill, addToHistory = true, source = 'unknown') {
...
function closeSkillModal(action = 'backdrop') {
    const modal = document.getElementById('skillModal');
...
function applyAccordionVisibility(options) {
    if (window.innerWidth > 767) return; // Только для мобильной версии
...
// ─────────────────────────────────────────────────────────────────────────────
// Parity-gate seam (A1-P4). NOT a public API.
//
