...
function reportStoreClose(outcome) {
    if (outcome.parked) {
        emitSignal('store.close', {
            outcome: 'complete',
            failure_class: 'none',
            close_ms: outcome.closeMs ?? null,
        });
        return;
    }
    // Nothing was open to close — the web channel, a page hidden before the
    // store opened, a second hide while already parked. Not an outcome of a
    // close, and deliberately silent (core/signals.js says why). Named rather
    // than inferred from a missing field: the three are the only reasons
    // parkNativeStore declines without trying, and a reader should be able to
    // check that against it.
    const declined = ['not-native', 'never-opened', 'already-parked'];
    if (declined.includes(outcome.reason)) return;
    const failureClass = storeFailureCode(outcome.reason);
    emitSignal('store.close', {
        outcome: 'failed',
        failure_class: failureClass,
        close_ms: outcome.closeMs ?? null,
...
/**
 * Wires the store to the page's lifecycle (FIU-DL-001).
 *
 * ONE TRIGGER, AND IT IS `visibilitychange` GOING HIDDEN. That event is what
 * Chromium fires when the Android window stops being visible, which is
 * `Activity.onStop` — a moment the process is still alive and JavaScript still
 * runs. `pagehide` is deliberately NOT used, and not used as a belt either:
 * DIA-DL-008 debt 8 asked for a disposition that does not depend on whether the
 * Android WebView fires `pagehide` before process death, and a second listener
 * on the very event the debt is about would invite the next reader to believe it
 * matters. What this rests on instead is executed on a device by
 * StoreLifecycleTest, which drives the activity to STOPPED and reads the log.
 *
 * WHAT IS NOT CLAIMED. Between `onStop` and a process kill Android may take the
 * process before the marker is written. Then the marker stays 0 and the next
 * open pays the integrity_check — which is the correct answer for a run that
 * really did die, and is exactly what every run did before this packet. No path
 * is worse than it was.
 *
 * The reopen reporter is registered here too, so a store that comes back after a
 * park reports itself on the same signal the boot open uses.
 */
export function wireStoreLifecycle() {
    onStoreReopened(reportStoreOpen);
    if (typeof document === 'undefined') return;
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState !== 'hidden') return;
        parkNativeStore().then(reportStoreClose);
    });
}
