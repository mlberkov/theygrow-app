...
export const SIGNAL_CODES = Object.freeze({
    // This list is SHARED across kinds — a field named here may carry any of
    // these in any kind — so widening it widens every kind at once. That is
    // accepted rather than overlooked: the alternative is a per-kind code table,
    // which buys tighter typing at the cost of a second place for a code to be
    // declared and a second thing to forget. What the widening cannot do is
    // admit free text, which is the property the whole surface rests on.
    //
    // changed_in: PPR-DL-002 — three codes REMOVED. `handed_off`, `refused` and
    // `no_transfer` were added by DIA-DL-001 for `history.handoff`, which is the
    // only kind that ever declared them, and that kind is gone with the
    // browser-to-native transfer. A code declared for a mechanism nothing ships
    // is a claim this surface makes about a thing that does not exist, and the
    // sharing rule above is exactly why it cannot be left lying here: an unused
    // code in a SHARED list is admissible in every kind that survives it.
    ...
    // changed_in: PPR-DL-002 — the `transport` and `refusal` lists are REMOVED.
    // DIA-DL-001 declared both for `history.handoff` and no other kind ever named
    // either field; `refusal` in particular was the vocabulary three places had to
    // agree on — HistoryTransferPlugin.java, TransferFormatError and this file —
    // and all three are retired together. Nothing inherits the codes: a refusal
    // vocabulary belongs to the mechanism that can refuse.
});
