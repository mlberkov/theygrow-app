 ...
 *
 * Разбор в try: испорченный JSON — не повод уронить страницу, а повод не
 * обещать перенос. Обещание, которое некому выполнить, — это ADR-015 ровно
 * наоборот.
 */
...
async function handoff() {
    ...
    try {
        ...
        if (prepared.mode === 'file') {
            ...
            // FIU-DL-002: сказано то, что происходит. Приложение больше не
            // предлагает перенос и не открывает выбор файла — поверхность
            // удалена вместе с предложением, — а файл собирается и сохраняется
            // ровно как раньше. Обещать «перенос продолжится там» значило бы
            // отправить родителя искать окно, которого нет.
            setStatus(
                'Файл с историей сохранён в загрузки.'
                    + ' Приложение его пока не принимает: переноса из браузера в нынешней'
                    + ' версии нет. Файл остался у вас, в браузере всё на месте.'
            );
            return;
        }
    ...
    if (fallbackRequested()) {
        ...
        setInstruction(
            'Приложение не открылось по ссылке. Нажмите кнопку — история сохранится файлом.'
                + ' Выбрать его в приложении пока нельзя: переноса из браузера в нынешней'
                + ' версии нет.'
        );
