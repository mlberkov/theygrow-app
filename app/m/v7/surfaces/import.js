 ...
 *
 * `unsolicited` — РАЗЛИЧАЕТ ДВА ВЫЗОВА, КОТОРЫЕ ДО FIU-P1 БЫЛИ ОДНИМ (FIU-DL-001).
 * Запуск приложения — момент, когда предложение уместно само по себе: родитель
 * только что открыл приложение, и экран под модалкой всё равно пустой. Возврат
 * из фона — не такой момент. Родитель был в дневнике, заблокировал телефон,
 * вернулся — и получал поверх своей работы полноэкранную (на телефоне
 * `.modal-content` это 100vw/100dvh) модалку переноса. Каждый раз: `delivered`
 * живёт в памяти модуля, `closeImportModal()` ничего не запоминает, и последняя
 * ветка ниже открывала модалку безусловно. Именно это владелец описал как
 * «приложение начинает сначала, с того же первого экрана» — экран и правда тот
 * самый, первый после установки.
 *
 * Поэтому по видимости выполняется ВСЁ, ради чего слушатель и заводился —
 * забрать то, что действительно пришло с страницы переноса, и объяснить отказ, —
 * и не выполняется единственное, чего родитель не просил: приглашение начать
 * перенос с нуля.
 *
 * Само предложение при запуске эта заплата НЕ трогает: убрать его совсем и
 * сделать свежую установку самодостаточной — отдельный пакет (P2).
 */
export async function offerImportIfPending({ unsolicited = true } = {}) {
    const handle = storeHandle();
    if (!handle) return;
...
export function wireImport() {
    ...
    //
    // FIU-P1: `unsolicited: false` — см. offerImportIfPending. Слушатель заведён
    // ради возврата С страницы переноса, и ровно это он и делает; предлагать
    // перенос заново тому, кто просто разблокировал телефон, он больше не будет.
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState !== 'visible') return;
        if (!isTransferAvailable()) return;
        offerImportIfPending({ unsolicited: false });
    });
}
