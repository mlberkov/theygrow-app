...
- **Landed in.** A1-P3.
- **Scope.** Covers assets referenced from HTML attributes (`href=`/`src=` values beginning with `/`), manifest icon sources, and the precache list. It does **not** cover: assets referenced only from JavaScript string literals — `fetch('/kb-v1.json')` in the shell is covered incidentally, via `OFFLINE_URLS`, and a future runtime-fetched asset that is not precached would be invisible; `url()` / `@import` references inside CSS (there are none today); `app/.dockerignore`, which the guard does not consult, so a future ignore pattern matching `m/` would make `COPY m/` ship nothing with this guard still green; and production `Content-Type`, which comes from the base image's `mime.types` and is observed only by the promotion-smoke curl in `docs/RUNBOOK.md`. The copy-forward / never-overwrite half of the `A1-DL-004` mount contract is a documented owner procedure, **not** a gate — nothing here can detect bytes changing at an already-published URL within one commit.

### A1-P4-INV-001 — The shipped module import graph is shipped and precached, transitively

- **Statement.** Every module the shell's `<script type="module">` entries import, transitively, resolves to a path the image ships **and** that is precached by name in `OFFLINE_URLS`. This extends `A1-P3-INV-001` past the HTML attribute surface: since A1-P4 the shell references exactly one JS entry (`/m/v1/app.js`) and every other module — `core/` since A1-P4, `surfaces/` since A1-P5 — is reachable only through an `import` statement, so both of that invariant's directions would stay green while a core module was absent from the `app/Dockerfile` `COPY` list (a production 404 that no test sees, because the parity server serves from disk) or from `OFFLINE_URLS` (a broken import graph on the first offline boot, because `cache.addAll` never fetched it). The walker fails **closed**: a dynamic `import()`, an indented module statement, or a bare specifier throws rather than being silently skipped, since the app is buildless and has no import map to resolve one.
- **Enforced by.** `app/tests/delivery-contract.spec.js` (`moduleSpecifiers` / `resolveSpecifier` / `moduleDependencies`, asserted in the `ship list — app/Dockerfile ships everything the shell needs` describe-block), run in the `contract` project of `app/playwright.config.js` via `scripts/parity-suite.sh` — invoked as the `parity` job in `.github/workflows/ci.yml`. Sources are the real `app/Dockerfile`, `app/sw.js` and the shipped module files on disk.
- **Landed in.** A1-P4.
- **Scope.** Covers **static** `import` / `export … from` specifiers in shipped JavaScript modules reached from a shipped HTML shell. It does **not** cover: dynamic `import()` (rejected loudly rather than followed — the parity suite's own test-seam import lives in dev/CI code, which this walker never reads); CSS `@import` / `url()` (there are none today, same gap as `A1-P3-INV-001`); assets fetched from JS string literals; or import correctness beyond existence — a module that ships and is precached but throws on evaluation is the parity suite's business, not this guard's.
