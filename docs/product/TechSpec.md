...
- **Prod vs dev (ADR-008 / ADR-039).** Production is **managed Cloud SQL Postgres + pgvector** (→ AlloyDB by load), to ADR-039's parameters — PostgreSQL 16, europe-west1, Enterprise, zonal, public IPv4 with a permanently **empty** authorized-networks list, daily backups plus PITR, and **one instance for all contours**. Since **A3-P2** the production `/api` build-config declares the attachment (`--add-cloudsql-instances`, `--set-secrets DATABASE_URL`, a dedicated runtime service account, `--max-instances 2`) and `GET /api/health/ready` is the one deployed route that opens a connection. The instance itself is created by an owner-run procedure whose steps 1–7 the owner **executed on 2026-07-29** — the instance, the production database, the application role and the secret exist; the deploy-side steps (merge, substitutions, migration, deploy, promotion) have **not** run, so the declared attachment has never been exercised. See `../RUNBOOK.md` "Production database enablement". Dev uses a local Postgres 16 + pgvector via the dev-only `docker-compose.yml`. dev vs prod is a config difference (the env-driven config reads connection/infra endpoints from the environment), not a code difference.
- **Staging (ADR-020 as amended, A2).** A third contour, **described but never raised**: a **separate staging database on the production Cloud SQL instance** — the accepted cost default and the ADR-039 one-instance rule, with isolation enforced by SQL grants and a least-privilege runtime identity rather than by a second instance (`L2-DL-001`). It is **not** a second source of truth: staging is to hold **no real family data**, only a committed synthetic `/export` v1 corpus (`L2-DL-002`), and no third-party provider is cleared for it — both per-egress clearance gates stay unset and fail-closed. Every step of standing the contour up is owner-run and none has been run; see `../RUNBOOK.md` "Staging contour".
