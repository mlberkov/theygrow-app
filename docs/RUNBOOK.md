...
- **The database wiring is not exercised by the deployed service.** No route constructs `Settings`, and `/api/health` opens no connection. `--set-secrets` validates only that the secret **exists** and that the runtime service account can **read** it — never that its contents are a usable URL, and never that the socket resolves. Until A2-P2 connects for real, the only positive evidence the wiring is correct is step 4's observed refusal and step 8's successful migration. Treat a green deploy as evidence of nothing here.
