...
- App icons (install surface): the brand icon/favicon set under `app/icons/` — manifest `purpose: "any"` 192/512 (`icon-logo-192-v2.png` / `icon-logo-512-v2.png`) + `purpose: "maskable"` 192/512 (`maskable-192-v2.png` / `maskable-512-v2.png`), the `<head>` `rel="icon"` PNGs `favicon-16/32.png` + `rel="apple-touch-icon"` `apple-touch-icon-v2.png` (180×180) — all precached by `app/sw.js` (`CACHE_VERSION v8`, which also delivers them to installed clients via the network-first SW + `#updateBanner` path). A discrete `favicon.ico` is intentionally **absent** (PNG-only): the PNG `rel="icon"` links cover the browser tab and the bare `/favicon.ico` probe 404s harmlessly. The assets use a new-filename scheme, so a `CACHE_VERSION` bump re-fetches them fresh (versioning-by-filename, vault ADR-024); superseded files stay on disk, dereferenced, and are never rewritten in place. See `ICON-DL-001`, and `A0-DL-001` for the `-v2` set.
