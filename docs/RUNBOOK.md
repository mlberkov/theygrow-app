...
- Module mount: `app/m/v{N}/` served at `/m/v{N}/` — the versioned home of everything extracted out of the shell (today `app.css`, `sw-register.js`, the app entry `app.js` and the `core/` modules it imports; the per-surface feature modules arrive in later A1 packets). The shell references only `app.css`, `sw-register.js` and `app.js`; everything under `core/` is reached through `import` statements, which is why the ship-list guard walks the import graph (`A1-P4-INV-001`). **Version-in-path**, so the bytes at a published URL never change: a content change ships as a **new** mount version and the old one stays on disk, byte-untouched. Served immutable by the generic static nginx location and precached by the SW; bumps are owner-run — see **Module mount** below. See `A1-DL-004`, `A1-P3-INV-001`.
...
2. **Bump to a new mount version (`/m/v{N+1}/`)** — copy-forward, never rename:
   ...
   3. `app/sw.js` — repoint **every** `/m/v{N}/…` entry in `OFFLINE_URLS`. Since A1-P4 that list covers the whole module graph (`app.js` plus each file under `core/`), not just the two files the shell names — `cache.addAll` is atomic and an unrepointed entry fails the install outright.
