...
5. **The eval precondition is a convention, not a mechanism.** For a backend change the intended gate is: **step 12 of the staging contour below — `theygrow-eval --mode staging` — has been run against staging and exited `0`** *before* this revision is promoted. Nothing enforces it. If the staging build fails outright, the production build still succeeds and still lands a promotable 0%-traffic revision; and nothing anywhere reads the eval's exit code or its scorecard on the way to the promote button. A failed or unrun staging eval is something the owner must **notice**, not something that blocks the button. Automating it is deferred (`L2-DL-003`).
