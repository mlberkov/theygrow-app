...
- **M3 in progress.**
  ...
  - **P1** — Episodic source schema + migration: the `source_messages` table (v1 wire mirror; engine-faithful TEXT ids; BIGINT `edit_seq`; PK + composite assertion-key UNIQUE; defensive `detected_route` CHECK; ADR-004 dual-timestamp; persona stub; reserved unindexed `vector(≤1536)` shell), migration tooling, the first real DB connection from product code, and DB-backed constraint tests run against a pgvector Postgres in CI. Decision `M3-DL-002`. **Current** (this packet).
